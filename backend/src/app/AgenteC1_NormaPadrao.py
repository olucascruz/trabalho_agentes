   ...
   async def executar(self, text: str) -> str:
    async def executar(self, text: str) -> str:
        return await self.pensar(
            f"Avalie o texto a seguir exclusivamente quanto ao domínio da norma-padrão da língua portuguesa, sem reescrever nada. "
            f"Identifique erros de gramática, ortografia, regência, pontuação e concordância, explicando cada ponto. "
